...
public static class Protocol
{
    ...
    public const byte MsgReject = 21; // u8 code (1 = bad secret, 2 = join token required/invalid on a Verified listing) — join refused; sent right before the transport closes so the client learns WHY across BOTH transports (a WebRTC DataChannel close carries no reason)
    ...
    public static byte[] BuildConstructorState(Simulation sim, byte team)
    {
        ...
        foreach (var r in rows)
        {
            ...
            w.Write(r.ShipId); // launched drone's ship id (0 while queued/producing) — ties a rendered ship to its station type
            n++;
        }
        w.Flush();
        var buf = ms.ToArray();
