...
| Script | What it does |
|--------|--------------|
| `package-clients.ps1` | **The release path.** Builds THIS OS's Velopack package — the Game Launcher (`launcher/`) plus the Godot client as an installer, a self-updating portable zip and the update feed — into `build/releases/<channel>`. The same script CI runs (`.github/workflows/release.yml`). `-Version <x.y.z>` (required), `-FakeGame` (stub game instead of a Godot export: seconds, not minutes), `-GameDir <export>` (reuse an export), `-HostArchOnly` (macOS: skip the universal build), `-ReleaseNotes <md>`. Needs `zstd` on macOS/Linux. See [`docs/RELEASING.md`](../docs/RELEASING.md). |
| `launcher-e2e.ps1` | Proves install → update → restart → play on this machine in about a minute: packs three launcher versions around the stub game, installs the first the way a player would (Windows: the real `Setup.exe --silent`), then drives the installed launcher headless through two real updates — one triggered by the game's UPDATE NOW exit code — asserting every step. Also what the **Package dry-run** workflow runs on all three OSes. `-KeepFiles` to inspect `build/launcher-e2e`. |
