...
requirement — below the ~2 u/s direction deadzone a ship counts as parked and docks on touch;
`CollisionConfig.DockApproachMinCosSq`/`DockDirectionDeadzoneSq` compile-time constants, sqrt-free
cos² compare on both peers). The client latches **dock-pending** the tick the predicate passes
(`PredictionController.DockPending`): statics and the collision thud are suppressed for the ghost
ticks predicted before `ShipGone` lands, so a dock never pops/thuds. Bases bake **fully solid** (no
...
- **Frequency:** Domain-specific
- **Key Files:**
  - `server/Sim/Simulation.Crew.cs` — roster (`CrewShip`, `_crewByCaptain`, `_seatOf`), queue drains + validation matrix, `ResolveTurretWeapon` (same tech gate + tier migration as mounts), bind in SpawnCombatShip / release in ApplyStructural + RemoveShipNow + ReturnToLobby, `TryPromoteGunner` (succession, called from the leave drain + `ExpireHeldOrphans`)
  - `server/Net/ClientHub.cs` + `ClientHub.Streams.cs` — handlers 17/18 (spawn team gate), riding gunner's AOI anchor follows the captain's ship, `CrewStream` (PerTeam, Reliable, `Events.CrewChanged || coarse`)
  - `server/Content/FactionsContentProjection.cs` — turret projection: authored → `Mount Gun`; an unauthored mesh turret node → `NoWeapon` + `NonMountable` (marker only)
  - `client/scripts/world/CrewStore.cs` — client roster (`SeatOf`, `ShipOf`, `Version`, `SeatId` = `T{n}`); `client/scripts/world/ShipRenderer.cs` — `RidingShipId` / `SetRiding` (sector follow via the spawn + warp seams)
  - `client/scripts/ui/ShipLoadout.Crew.cs`, `CommandSidebar.cs`, `ui/GunnerStrip.cs` — CREWED SHIPS cards, TURRET STATIONS + turret arsenal mode, the CREWING view, the in-flight gunner strip
  - `shared/TurretAim.cs` — THE arc/rest/gimbal rule (`Clamp`, `Rest`, `FromGimbal`, `SpreadBarrel`, `DefaultSlewDeg`, `SlewLimit`/`SlewCapacity`/`SlewWindowSec` — the slew bucket); `server/Sim/Simulation.Firing.cs` `TryFireTurrets` — held gunner input → arc clamp → `FireBolt` with the gunner as attacker; `ClientHub` `MsgTurrets` per-client AOI build (in view of the anchor, or riding it)
  - `client/scripts/TurretController.cs` (gunner gimbal + input send + own-bolt prediction), `CameraRig.cs` gun cam, `TurretBarrelView.cs`, `ui/TurretReticle.cs`, `world/TurretStations.cs` (seat→slot POCO), `ShipRenderer.ApplyTurrets` (remote turret bolts + barrels)
  - `tests/CrewTest`, `tests/CrewStoreTest`
- **Related:** [[Per-Ship Weapon Loadout (mount overrides)]], [[Hardpoint]], [[AOI (Area of Interest)]]
- **Notes:** Docked captains only — no mid-flight boarding. Seat ids on the wire are the turret `HardpointDef.Index` (the sim maps to a station slot). Turret guns are exempt from `payload-capacity` and never drop as salvage. A gunner's own MsgSpawn auto-vacates the seat; join-while-seated = move; a pilot ADVERTISING their own hull who claims a seat elsewhere has that advertisement withdrawn by the SUCCESSFUL claim (its gunners freed; a refused claim costs nothing) — the hangar advertises the selected hull on open, so refusing made ＋ JOIN a dead button; the captain's DEATH ejects every seated gunner into their own escape pod (no EJ scored for them) and dissolves the crew, while a clean DOCK keeps the record (ShipId 0, seats held, joinable again) until a gunner leaves or the captain advertises a different class; a captain who LEAVES the game (MsgBye, or a held orphan whose reconnect grace expired) does NOT take a LAUNCHED hull with them — `Simulation.TryPromoteGunner` PROMOTES the seated gunner in the lowest station slot to captain (seat vacated, held input and crew-record key migrated exactly as a reclaim does, `_clientInfo` copied over, no `MigrateStats` so they keep their own scoreboard row, no ShipGone and nothing scored) and the other gunners stay seated; a crew with no launched ship, or with nobody aboard, still dissolves; a team change clears crew participation; a captain reclaim migrates the roster key. Riding gunners get no reconnect grace (a shipless drop is a clean leave). A fresh/reconnected client first sees the roster on the next `CrewChanged` or the coarse keepalive (~0.5 s). PIG bombers fly authored stations, unbound. Turret fire is NOT part of the deterministic flight step (no exact-tick input ring); the gunner's client ignores its own seat in `MsgTurrets` (it predicts its own aim and bolts). Vision signature and PIG aggro read `max(LastFireTick, LastTurretFireTick)`. A seated gunner gets the PILOT's HUD (`HudSubject` seam: aim reticle, lead indicator, Tab target cycle, system ring, prograde marker, weapons panel) fed from the ridden hull and the seat's gun; the gun cam is a pilot-style FREE LOOK (`client/scripts/world/TurretLook.cs`: the mouse yaws/pitches the look basis about its own axes — no azimuth/elevation state — carrying straight over the zenith until the arc edge rotates the whole basis back). The camera IS that look, and so is the gun: constrained only by the mount's position, the hull's pose, the arc fence and the station's slew SPEED, which limits the look's SUSTAINED turn rate through the slew bucket (small motions are never limited; motion past the bucket is dropped, not banked). That limit is the only turret physics left, and the client is the one that applies it. **Turret feel rig:** `scripts/turret-test.ps1` (server + a still captain client + your gunner client, `--turret-test=captain|gunner|auto`, `TurretTestProbe` readout / `[turret-stats]` log, env tuning `TURRET_GAIN` / `TURRET_SLEW_DEG` / `TURRET_SLEW_WINDOW`; flag + env honoured in DEBUG builds only, `OS.IsDebugBuild()`); `-Auto` is the scripted linearity + gunner-chat check.
