...
- **Key Files:**
  ...
  - `shared/TurretAim.cs` — THE arc/rest/gimbal rule (`Clamp`, `Rest`, `FromGimbal`, `SpreadBarrel`, `DefaultSlewDeg`); `server/Sim/Simulation.Firing.cs` `TryFireTurrets` — held gunner input → arc clamp → `FireBolt` with the gunner as attacker; `ClientHub` `MsgTurrets` per-client AOI build (in view of the anchor, or riding it)
