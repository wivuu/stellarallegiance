...
scripts/run-server.ps1 -Local --autostart          # run in background, wait for :8090 LISTEN

# 2. Self-driving client with evidence capture (quits by itself after the shot):
$env:MOVIE_RESOLUTION='1280x720'; scripts/run-client.ps1 -Local -WriteMovie <scratch>/smoke.avi `
  --autofly --combat-test -- --ui-shot=<scratch>/live.png --ui-shot-delay=14
```
...
- `--ui-shot` saves a PNG after the delay then quits cleanly — this also finalizes
  `-WriteMovie`. Extract frames: `ffmpeg -ss <t> -i smoke.avi -frames:v 1 f.png`.
...
- Content probes without touching the repo: copy `server/Content/core` to scratch, edit the
  ...
  point the client at it with `$env:SIM_PORT=8099; scripts/run-client.ps1 -Local ...`).
