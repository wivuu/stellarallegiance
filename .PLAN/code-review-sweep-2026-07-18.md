...
- ✅ DONE 2026-07-19 (follow-up decomposition session): WorldRenderer TeamStateStore extraction (M24) & god-class split (M27) — WorldRenderer 3468→754 lines, 24 collaborators in client/scripts/world/ (see ADDENDUM + `.claude/plans/rosy-wiggling-sky.md`)
- ⏭️ STILL DEFERRED (see ADDENDUM): Lobby factory-kit→ui-lib (M28)
...
### ✅ Completed in a follow-up decomposition session (2026-07-19) — M24 + M27
Both were done properly (not the `partial`-file shortcut the loop pass warned against, which the user explicitly rejected as "papering over the wrong abstractions"). See the plan `.claude/plans/rosy-wiggling-sky.md`. `WorldRenderer` was decomposed into **24 single-responsibility collaborators** in `client/scripts/world/`; the god-class shrank **3468 → 754 lines** and is now a thin coordinator (node-tree parent + `_Ready` wiring + `_Process`/`Reset` fan-out + the warp/sector/home cluster). Executed in 4 user-gated milestones (A: stores/services + M24; B: static-world producers; C: dynamic/combat layer; D: environment + finalize), each compile-gated + determinism-gated (FlightModelTest/MineTest PASS) and visually verified by the user in the running client.
- **M24 — `TeamStateStore`.** Extracted to `client/scripts/world/TeamStateStore.cs` as a pure POCO, decoupled from Godot via `ITickSource` (MatchClock) + `IShipCostSource` (DefRegistry). Every reader rewired to `_world.TeamState.*`. This directly answered the loop pass's "no headless test to catch it" objection: a new **`tests/TeamStateStoreTest`** (50 assertions, in the solution) now covers the reconcile/gate/progress logic in headless CI.
- **M27 — god-class split into real classes.** Concerns became genuine collaborators with narrow injected dependencies (`RenderSeams.cs`: `IShipQuery`/`IEffectSink`/`IWarpDriver`/`IBuildQuery`/… ), NOT `partial` files. Cross-subsystem coupling resolved via query interfaces + a `ClipCache` seam + a `restTransparency` node-meta; the Ships↔Bolts/Collision cycles resolve by construction order + coordinator back-edges. Consumer call-sites rewired to `_world.<Subsystem>.*`.

### ⏭️ Not fixed — deferred (pure organization; recommend a deliberate, visually-verified session)
Readability-only, on the highest-churn client file, verifiable only by visual parity in the running Godot client. Risk/benefit does not favour doing it blind inside an automated sweep.
