...
WorldConfig FieldConfig(
    WorldMiningTuning mining,
    WorldSeedingTuning? seeding = null,
    int? he3Count = null,
    float? richness = null,
    float radius = 1500f,
    float density = 3f,
...
// ---- 1. Same seed ⇒ identical class + capacity assignment for every rock. ----
{
    ...
    if (same)
        for (int i = 0; i < a.Asteroids.Count; i++)
        {
            ...
            if (
                ...
                || ra.Radius != rb.Radius
                || oa.Class != ob.Class
                || oa.OreCapacity != ob.OreCapacity
                || oa.OreRemaining != ob.OreRemaining
                || oa.CurrentRadius != ob.CurrentRadius
            )
            {
                same = false;
                break;
    ...
    Check(
        same,
        "same seed ⇒ byte-identical class + capacity + variant + radius for every rock",
        "ore assignment is not seed-deterministic"
    );
    ...
    foreach (var r in a.Asteroids)
    {
        ...
        if (Array.IndexOf(AsteroidShapes.PoolFor(cls), r.Variant) < 0)
        {
            poolOk = false;
            poolDetail =
                $"rock {r.Id} class {cls} variant {r.Variant} ({AsteroidShapes.NameForIndex(r.Variant)}) not in class pool";
            break;
        }
    }
...
// ---- 2. THE CANARY: ore-knob tuning never perturbs the rock/aleph LAYOUT for a pinned seed. ----
{
    ...
    var seedLoud = new WorldSeedingTuning
    {
        He3PerSector = 9999,
        SpecialPerSector = 5,
        HomeSpecialChance = 1f,
    };
    ...
    Check(
        rocksIdentical,
        "rock positions/rotations byte-identical across wildly different mining knobs",
        "ore assignment perturbed the rock layout (drew on the shared RNG!)"
    );
    ...
    if (alephsIdentical)
        for (int i = 0; i < w1.Alephs.Count; i++)
        {
            ...
            if (
                ...
                || ga.Pos.Y != gb.Pos.Y
                || ga.Pos.Z != gb.Pos.Z
                || ga.PartnerPos.X != gb.PartnerPos.X
                || ga.PartnerPos.Y != gb.PartnerPos.Y
                || ga.PartnerPos.Z != gb.PartnerPos.Z
            )
            {
    ...
    Check(
        alephsIdentical,
        "aleph gate positions byte-identical across wildly different mining knobs",
        "ore assignment perturbed the aleph layout (drew on the shared RNG!)"
    );
    ...
    Check(
        he3Off == 0 && he3Loud > he3Off,
        $"mining knobs still take effect (He3: off={he3Off}, loud={he3Loud})",
        "mining knobs had no effect — the canary would be vacuous"
    );
}
...
// ---- 3. Per-sector He3 count: the guaranteed count, ≤ rock count, across several seeds (stock knobs). ----
{
    ...
    Check(
        allOk,
        $"per-sector He3 count is the guaranteed he3-per-sector ({want}) and never exceeds the rock count (6 seeds)",
        $"He3 count off: {detail}"
    );
}
...
// ---- 4. A per-sector he3-count override BEATS the world default. ----
{
    ...
    Check(
        he3 == 5,
        $"sector he3-count override forces exactly 5 He3 (world default was 1) — got {he3}",
        $"per-sector override ignored: got {he3}, want 5"
    );
}
...
//          tests 3 and 7b pin those on this same stock config. ----
{
            ...
            for (int i = 0; i < rocks.Count && gapOk; i++)
            {
                ...
                for (int j = i + 1; j < rocks.Count; j++)
                {
                    ...
                    if ((rocks[i].Pos - rocks[j].Pos).LengthSquared() < need * need)
                    {
                        gapOk = false;
                        detail =
                            $"seed {seed} rocks {rocks[i].Id}/{rocks[j].Id}: dist {(rocks[i].Pos - rocks[j].Pos).Length():F1} < {need:F1}";
                        break;
                    }
                }
                foreach (var b in w.Bases)
                {
                    ...
                    if ((rocks[i].Pos - b.Pos).LengthSquared() < need * need)
                    {
                        clearOk = false;
                        detail =
                            $"seed {seed} rock {rocks[i].Id} vs base {b.Id}: dist {(rocks[i].Pos - b.Pos).Length():F1} < {need:F1}";
                    }
                }
            }
        }
    ...
    Check(
        s.RockMinGap > 0f && s.BaseClearance > 0f,
    ...
    Check(
        gapOk,
        "every same-sector rock pair honours rock-min-gap (surface-to-surface, 3 seeds)",
        $"rock gap violated: {detail}"
    );
    Check(
        clearOk,
        "every rock honours base-clearance around its sector's bases (3 seeds)",
        $"base clearance violated: {detail}"
    );
    ...
    const float radius = 1500f,
        density = 3f;
    int expected = (int)
        MathF.Round(density * off.FieldAreaDensity * MathF.PI * (radius * off.FieldFillFrac) * (radius * off.FieldFillFrac));
    ...
    Check(
        wOn.Asteroids.Count >= (int)(expected * 0.9f),
...
// ---- 6. Capacity: size-scaled, clamped into [ore-capacity-min, ore-capacity-max], richness within band. ----
{
    ...
    Check(
        he3Seen >= 4,
        $"capacity test observed {he3Seen} He3 rocks",
        $"too few He3 rocks for the capacity test ({he3Seen})"
    );
    Check(
        exactOk,
        "every He3 capacity == the size-scaled, clamped formula (pure function of radius)",
        "a He3 capacity did not match the size-scaled formula"
    );
    Check(
        boundsOk,
        "every He3 capacity lands within [ore-capacity-min, ore-capacity-max] (hard clamp)",
        "a He3 capacity fell outside the clamp band"
    );
    Check(
        radiusStateOk,
        "He3 rocks start full (OreRemaining == OreCapacity) at their spawn radius (CurrentRadius == radius)",
        "He3 rock initial ore state wrong"
    );
    ...
    foreach (var r in w1.Asteroids)
    {
        if (w1.RockOre[r.Id].Class != RockClass.Helium3)
            continue;
        float c1 = w1.RockOre[r.Id].OreCapacity,
            c2 = w2.RockOre[r.Id].OreCapacity;
        if (c2 < c1 - 1e-3f)
            richMono = false;
        if (c2 > c1 + 1e-3f)
            richStrict = true;
    ...
    Check(
        richMono && richStrict,
        "OreRichnessMult raises capacity (richer ≥ leaner, strictly greater for some rock)",
        "OreRichnessMult did not raise capacity"
    );
    ...
    foreach (var r in wFull.Asteroids)
    {
        if (wFull.RockOre[r.Id].Class != RockClass.Helium3)
            continue;
        wideSeen++;
        float full = wFull.RockOre[r.Id].OreCapacity,
            half = wHalf.RockOre[r.Id].OreCapacity;
        if (MathF.Abs(half - full * 0.5f) > full * 1e-4f + 1e-3f)
            halfOk = false;
    }
    Check(
        wideSeen >= 4 && halfOk,
        "OreRichnessMult scales capacity linearly within the band (0.5 = exactly half)",
        "OreRichnessMult did not scale capacity proportionally within the band"
    );

    // Size scaling: capacity is monotonic non-decreasing in rock radius (bigger rock ⇒ ≥ ore).
    var he3rocks = w1.Asteroids.Where(r => w1.RockOre[r.Id].Class == RockClass.Helium3).OrderBy(r => r.Radius).ToList();
    bool sizeMono = true;
    ...
    Check(
        sizeMono,
        "capacity is monotonic non-decreasing in rock radius",
        "a bigger He3 rock held less ore than a smaller one"
    );
}
...
// ---- 7. RockCurrentRadius / RockClassOf fallbacks for non-He3 + unknown ids. ----
{
    ...
    Check(
        w.RockCurrentRadius(nonHe3.Id) == nonHe3.Radius,
        "RockCurrentRadius returns the static spawn radius for a non-He3 rock",
        "non-He3 RockCurrentRadius mismatch"
    );
    var cls = w.RockClassOf(nonHe3.Id);
    Check(
        cls is RockClass.Carbonaceous or RockClass.Silicon or RockClass.Uranium or RockClass.Regolith,
        $"a non-He3 rock is a valid non-He3 class ({cls})",
        $"non-He3 rock has unexpected class {cls}"
    );
    // Unknown id: class defaults to Carbonaceous.
    Check(
        w.RockClassOf(0xDEADBEEF) == RockClass.Carbonaceous,
        "RockClassOf defaults to Carbonaceous for an unknown id",
        "unknown-id class default wrong"
    );
}
...
// ---- 7b. Rarity: a sector is overwhelmingly common (Regolith); specials obey special-per-sector. ----
{
    ...
    foreach (var r in w.Asteroids)
    {
        ...
        if (c == RockClass.Helium3)
            e.he3++;
        else if (c is RockClass.Carbonaceous or RockClass.Silicon or RockClass.Uranium)
        {
            e.special++;
            totalSpecial++;
        }
        else if (c is RockClass.Regolith)
        {
            e.common++;
            totalCommon++;
        }
    ...
    foreach (var (sector, e) in bySector)
    {
        ...
        if (e.special > 1)
        {
            specialsCapped = false;
            detail = $"sector {sector}: {e.special} specials";
        }
        if (e.common > 0)
            commonsExist = true;
        // He3 pinned at 4 (or the whole sector if it has fewer than 4 rocks).
        if (e.he3 != Math.Min(4, n))
        {
            he3PinnedOk = false;
            detail = $"sector {sector}: he3={e.he3}, n={n}";
        }
    }
    ...
    Check(
        commonsExist && totalCommon > totalSpecial,
...
// ---- 7c. Special (rare) rocks are oversized by SpecialRockRadiusMult; He3 + commons keep their size. ----
{
    ...
    foreach (var r3 in w3.Asteroids)
    {
        ...
        {
            specialsSeen++;
            if (MathF.Abs(r3.Radius - r1 * 3f) > 1e-3f)
            {
                specialsScaled = false;
                detail = $"special {r3.Id}: {r1:F2}→{r3.Radius:F2} (want ×3)";
            }
            if (MathF.Abs(w3.RockCurrentRadius(r3.Id) - r3.Radius) > 1e-3f)
            {
                specialsScaled = false;
                detail = $"special {r3.Id} CurrentRadius != Radius";
            }
        }
        else if (MathF.Abs(r3.Radius - r1) > 1e-3f)
        {
            othersUnchanged = false;
            detail = $"non-special {r3.Id} ({cls}) changed {r1:F2}→{r3.Radius:F2}";
        }
    }
    Check(
        specialsSeen >= 3,
        $"oversize test observed {specialsSeen} special rocks",
        $"too few special rocks ({specialsSeen})"
    );
    Check(
        specialsScaled,
        "special (rare) rocks scale by SpecialRockRadiusMult (×3) in radius + CurrentRadius",
        $"special oversize wrong: {detail}"
    );
    Check(
        othersUnchanged,
        "He3 + common rocks keep their rolled radius (only specials oversize)",
        $"a non-special rock changed size: {detail}"
    );
}
...
MapDef HomeMap(int? sector2He3 = null, int? sector2Specials = null) =>
    new()
    {
        Name = "HomeEconTest",
        Sectors = new List<MapSectorDef>
        {
            new()
            {
                ...
                Radius = 1500,
                Asteroids = "field",
                Garrison = new GarrisonDef { Team = 0 },
            },
            new()
            {
                Id = 1,
            ...
            new()
            {
                ...
                Radius = 1500,
                Asteroids = "field",
                Garrison = new GarrisonDef { Team = 1 },
                He3Count = sector2He3,
                SpecialCount = sector2Specials,
            },
        },
        Links = new List<List<uint>>
        {
            new() { 0, 1 },
            new() { 1, 2 },
        },
...
//          contested (non-garrison) sector keeps he3-per-sector (4), and a map's OWN he3-count wins. ----
{
    ...
    var he3 = He3Counts(w);
    Check(
        cfg.Seeding.He3PerHomeSector == 2 && cfg.Seeding.He3PerSector == 4 && cfg.Seeding.HomeSpecialChance == 0f,
...
//          case is a real interior pick (never a fallback), and the OFF case proves the knob is not vacuous. ----
{
    ...
    for (ulong seed = 1; seed <= 60; seed++)
    {
        ...
        if (clrOn != float.MaxValue)
        {
            specialsSeen++;
            worstOn = MathF.Min(worstOn, clrOn);
        }
...
{
    Check(
        stockCfg.Seeding.SpecialEdgeMargin > 0f,
...
// ---- 8. SetOreRemaining shrink helper: volume-proportional radius toward the floor. ----
{
    ...
    Check(
        w.RockCurrentRadius(nonHe3.Id) == before,
        "SetOreRemaining is a no-op for a non-He3 rock (never shrinks)",
        "a non-He3 rock shrank"
    );
}
...
WorldConfig HarvestConfig(float rate = 40f, float floorFrac = 0.4f, float standoff = 60f) =>
    FieldConfig(
        new WorldMiningTuning
        {
            HarvestRatePerSecond = rate,
            ShrinkFloorFrac = floorFrac,
            MinerStandoff = standoff,
            OreCapacityMin = 800f,
            OreCapacityMax = 3000f,
        },
...
// ---- 9. HarvestStep moves rate·dt per tick exactly; ore is conserved (rock loses what the hold gains). ----
{
    ...
    Check(
        oreOk && rockOk && conserved && returnOk,
...
// ---- 10. Full hold stops transfer; an out-of-range miner harvests nothing; a non-miner hull holds nothing. ----
{
    ...
    Check(
        movedFull == 0f && w.RockOre[he3.Id].OreRemaining == remBefore,
        "a full hold transfers nothing (rock untouched)",
        "a full-hold miner still harvested"
    );
    ...
    Check(
        sim.HarvestStep(far, he3.Id, 0.05f) == 0f,
        "an out-of-range miner harvests nothing",
        "an out-of-range miner still harvested"
    );

    // A non-miner hull (class 0 scout — 0 ore-capacity) can never harvest.
    var scout = new Simulation.ShipSim
    {
        Class = 0,
        SectorId = he3.SectorId,
        State = { Pos = he3.Pos },
    };
    Check(
        sim.HarvestStep(scout, he3.Id, 0.05f) == 0f,
        "a non-miner hull (0 ore-capacity) harvests nothing",
        "a non-miner hull harvested ore"
    );
    ...
    Check(
        sim.HarvestStep(m, nonHe3.Id, 0.05f) == 0f,
        "a cosmetic (non-He3) rock yields no ore",
        "a non-He3 rock was harvested"
    );
}
...
// ---- 11. Depletion: ore floors at 0 and CurrentRadius clamps to the shrink floor; an empty rock gives no more. ----
{
    ...
    bool floored = w.RockOre[he3.Id].OreRemaining == 0f && MathF.Abs(w.RockCurrentRadius(he3.Id) - floor) < 1e-2f;
    m.Ore = 0f;
...
// ---- 12. RockBodies collision scale tracks CurrentRadius ABSOLUTELY (no compounding across harvests). ----
{
    ...
    Check(
        track1 && track2 && spawnFixed && cur2 < cur1,
...
// ---- 13. RocksChangedThisStep: flagged on a real change, skipped on no-ops, cleared each sim step. ----
{
    ...
    Check(
        noopClean && changedFlagged && nonHe3Clean,
    ...
    sim.Step();
    Check(
        w.RocksChangedThisStep.Count == 0,
        "the sim step clears RocksChangedThisStep",
        "changed-rock set not cleared per step"
    );
}
...
// ---- 14. Two independent sims: same seed + identical scripted harvest ⇒ byte-identical results. ----
{
    ...
    var b = Run(31415);
    Check(
        a.ore == b.ore && a.rem == b.rem && a.cur == b.cur,
...
// flips once that radius is routed through RockCurrentRadius (this stream's change) instead of the spawn radius. ----
{
    ...
    Check(
        cur < spawn && hitsSpawn && !hitsCurrent,
...
// ---- 16. Wire: MsgRockUpdate carries the live (mined-down) radius + orePct for a changed rock. ----
{
    ...
    Check(
        tagged
            && countOne
            && id == he3.Id
            && rad == w.RockCurrentRadius(he3.Id)
            && rad < spawn
            && pct == expectPct
            && ms.Position == frames[0].Length,
...
WorldMiningTuning LoopMining() =>
    new()
    {
        HarvestRatePerSecond = 400f,
        OreCapacityMin = 5000f,
        OreCapacityMax = 6000f,
        OffloadDelaySeconds = 1f,
        MinerStandoff = 60f,
    };
...
// ---- 18. Claims: two miners split rocks (rule a); miners > eligible rocks ⇒ they double up. ----
{
    ...
    Check(
        rows.Count == 2
            && rows[0].TargetRockId != 0
            && rows[1].TargetRockId != 0
    ...
    Check(
        rows1.Count == 2 && rows1[0].TargetRockId != 0 && rows1[0].TargetRockId == rows1[1].TargetRockId,
...
//          harvests, and hauls the ore 2 hops home. ----
{
    ...
    var row = sim.MinerSlotsView()[0];
    Check(
        row.Idle && row.Ship is null && noticesSeen.Any(t => t.Contains("idle")),
    ...
    Check(
        reachedField,
        "an authorized far sector sends the miner TWO hops out to the rock field",
        "miner never reached the authorized field"
    );
    Check(
        offloaded && w.TeamStates[0].Credits > credits0,
...
// ---- 21. Purchase: cap enforced, cost charged through the Stage-2 buy seam. ----
{
    ...
    Check(
        w.TeamStates[0].Credits == 10_000 - 3 * minerCost,
...
// ---- 22. A miner hull can never be a PLAYER ship (MsgSpawn for class 4 is dropped). ----
{
    ...
    for (int i = 0; i < 5; i++)
        sim.Step();
    Check(
        sim.ShipIdOf(7) == 0 && w.TeamStates[0].Credits == 10_000,
...
// ---- 23. Fog gate: with fog ON and the rock undiscovered, the miner stays idle-docked. ----
{
    ...
    Check(
        !violated && (launchedAfter || !discovered),
...
// ---- 24. Kill-switch OFF + buy phase-gating (disabled / lobby / insufficient credits). ----
{
    ...
    Check(
        off.MinerCount(0) == 0,
        "MinersEnabled=false before StartMatch seeds NO slots",
        "a mining-disabled server still seeded a miner"
    );
    off.EnqueueMinerBuy(0);
    StepAndCollect(off);
    Check(
        off.MinerCount(0) == 0 && noticesSeen.Any(t => t.Contains("disabled")),
        "a buy on a mining-disabled server is refused politely (count stays 0)",
        "a disabled server accepted a miner buy"
    );
    ...
    Check(
        lobby.MinerCount(0) == 0 && noticesSeen.Any(t => t.Contains("during a match")),
        "a buy in the lobby is refused (miners only during a match)",
        "the lobby accepted a miner buy"
    );
    ...
    Check(
        sim.MinerCount(0) == 1
            && w.TeamStates[0].Credits == before
            && noticesSeen.Any(t => t.Contains("Not enough credits")),
        "a buy with too few credits is refused (count + credits unchanged)",
        "an unaffordable buy was not refused"
    );
}
...
// ---- 26. Base-destroyed reroute: a full miner retargets a surviving base; none left ⇒ it holds. ----
{
    ...
    for (int i = 0; i < 200 && miner is null; i++)
    {
        sim.Step();
        miner = sim.MinerSlotsView()[0].Ship;
    }
    ...
    if (miner is not null)
    {
        ...
        int guard = 0;
        while (miner.Ore < hold - 1e-3f && guard++ < 600)
        {
            Park();
            sim.Step();
        }
        Check(
            miner.Ore >= hold - 1e-3f,
            "the miner filled its hold (reroute pre-condition)",
            "the miner never filled (reroute)"
        );
        for (int i = 0; i < 8; i++)
        {
            Park();
            sim.Step();
        } // let the brain flip to ToBase

        var baseA = w.Bases.First(b => b.SectorId == 0);
        var baseB = w.Bases.First(b => b.SectorId == 1);
        var row = sim.MinerSlotsView()[0];
        Check(
            row.State == "ToBase" && row.TargetBaseId == baseA.Id,
            "a full miner heads to the nearest (same-sector) base A",
            $"full-miner target wrong (state {row.State}, target {row.TargetBaseId})"
        );
        ...
        Check(
            sim.MinerSlotsView()[0].Ship is not null && sim.MinerSlotsView()[0].TargetBaseId == baseB.Id,
            "with base A destroyed inbound, the miner retargets the surviving base B",
            $"reroute wrong (target {sim.MinerSlotsView()[0].TargetBaseId})"
        );
        ...
        Check(
            sim.MinerSlotsView()[0].Ship is not null && sim.MinerSlotsView()[0].TargetBaseId == 0,
            "with no friendly base left the full miner holds (no offload, no crash)",
            "the miner did not hold with no base left"
        );
    }
}
...
// ---- 27. Repurchase after a kill restores the count (slot is GONE until rebought; no pod). ----
{
    ...
    Check(
        drone is not null && sim.MinerCount(0) == 1,
        "a miner is live before the kill (repurchase test)",
        "no live miner before the repurchase test"
    );
    drone!.Health = 0f;
    sim.Step(); // ResolveDeath → KillMiner
    Check(
        sim.MinerCount(0) == 0 && !sim.Ships.Any(s => s.IsPod && s.Team == 0),
        "a killed miner removes its slot and leaves NO pod",
        $"kill handling wrong (count {sim.MinerCount(0)})"
    );
    w.TeamStates[0].Credits = 10_000;
    sim.EnqueueMinerBuy(0);
    sim.Step();
...
// ---- 29. Same-team ship-ship collision: friendly ships bounce AND take collision damage. ----
{
    ...
    Check(
        ...
        "two friendly miners are live (same-team collision pre-condition)",
    ...
    if (a is not null && b is not null)
    {
        ...
        Check(
            separating,
            "a same-team head-on resolves to separating velocities (impulse applied)",
            $"friendly pair still closing after the step (rel {rel.X:F1},{rel.Y:F1},{rel.Z:F1})"
        );
        Check(
            a.Health < hpA && b.Health < hpB,
...
// ---- 30. A ship bump knocks a Harvesting miner back to ToRock: beam reset, cargo + claim kept. ----
{
    ...
    Check(
        a is not null && b is not null,
        "a Harvesting miner + a second friendly are live (bump pre-condition)",
    ...
    if (a is not null && b is not null)
    {
        ...
        var row = sim.MinerSlotsView()[0];
        Check(
            row.State == "ToRock" && !a.IsHarvesting,
        ...
        Check(
            a.Ore >= oreBefore - 1e-3f && row.TargetRockId == rockId,
...
// ---- 31. An asteroid bounce disrupts the beam too (same stamp seams as ship-ship). ----
{
    ...
    Check(
        ship is not null,
        "a miner reached Harvesting (asteroid-bump pre-condition)",
    ...
    if (ship is not null)
    {
        ...
        sim.Step();
        Check(
            sim.MinerSlotsView()[0].State == "ToRock",
...
Simulation? retreatSim = null; // test 33 continues this sim through dock + relaunch
{
    ...
    if (ship is not null)
    {
        ...
        Check(
            row.State == "ToBase" && row.TargetBaseId != 0 && row.TargetRockId == 0,
...
if (retreatSim is { } rsim)
{
    ...
    Check(
        docked && !repicked,
        "the retreating miner flew home without re-picking a rock",
    ...
    Check(
        relaunched is not null
            && MathF.Abs(relaunched.Health - minerMaxHull) < 1e-3f
...
// ---- 34. Sub-threshold damage does NOT retreat (hull above retreat-health-frac keeps mining). ----
{
    ...
    Check(
        ship is not null,
        "a miner reached Harvesting (sub-threshold pre-condition)",
        "never reached Harvesting (sub-threshold)"
    );
    if (ship is not null)
    {
        ...
        var row = sim.MinerSlotsView()[0];
        Check(
            row.State != "ToBase" && !noticesSeen.Any(t => t.Contains("Miner damaged")),
...
// the live Iron faction (mining-rate ×0.85, mining-capacity ×0.75), and both flow through HarvestStep.
{
    ...
    Check(
        MathF.Abs(moved - wantRate) < 1e-3f,
    ...
    Check(
        movedFull == 0f && MathF.Abs(movedNearFull - 0.5f) < 1e-3f,
...
//      MinerSlot. Restores the delay this suite zeroed above. ----
{
    ...
    try
    {
        ...
        sim.EnqueueMinerBuy(0);
        for (int i = 0; i < 15; i++)
            sim.Step(); // drain the buy + a few ticks (well under the 60-tick delay)
        ...
        Check(
            sim.MinerCount(0) == 2 && sim.MinerSlotsView().Count == 1 && producing,
        ...
        for (int i = 0; i < 70; i++)
            sim.Step(); // step past the 60-tick order-time → graduate
        bool graduated = sim.MinerSlotsView().Count == 2 && !sim.ConstructorStatesView().Any(r => r.ProducesMiner);
        Check(
            graduated && sim.MinerCount(0) == 2,
