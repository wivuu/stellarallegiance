...
var lobby = new Lobby();
Check(
    lobby.CommanderOf(0) == -1 && lobby.CommanderOf(1) == -1,
    "empty lobby has no commanders",
    "empty lobby commander not -1"
);
...
lobby.SetTeam(3, 1);
Check(
    lobby.CommanderOf(0) == 2,
    "first joiner seeds team 0 commander (Bob, id 2)",
    $"team 0 commander wrong ({lobby.CommanderOf(0)})"
);
Check(
    lobby.CommanderOf(1) == 3,
    "first joiner seeds team 1 commander (Cid, id 3)",
    $"team 1 commander wrong ({lobby.CommanderOf(1)})"
);

// ---- 2. Fall-through on leave: command falls to the lowest remaining id on the side. ----
lobby.Remove(2);
Check(
    lobby.CommanderOf(0) == 1,
    "commander leaving falls to next-lowest id (Alice, id 1)",
    $"fall-on-leave wrong ({lobby.CommanderOf(0)})"
);
...
Check(
    lobby.CommanderOf(0) == 4,
    "commander switching sides vacates old side (Dee, id 4, takes over)",
    $"fall-on-switch wrong ({lobby.CommanderOf(0)})"
);
Check(
    lobby.CommanderOf(1) == 3,
    "defector does NOT displace the other side's commander",
    $"team 1 commander displaced ({lobby.CommanderOf(1)})"
);
...
Check(
    lobby.CommanderOf(0) == 4,
    "returning senior pilot does not displace the sitting commander",
    $"return displaced commander ({lobby.CommanderOf(0)})"
);
...
Check(
    lobby.CommanderOf(0) == 1,
    "later join does not displace a manual assignment",
    $"join displaced manual commander ({lobby.CommanderOf(0)})"
);
...
Check(
    lobby.CommanderOf(0) == 1,
    "refused assignments leave state untouched",
    $"refused assignment mutated state ({lobby.CommanderOf(0)})"
);
...
Check(
    lobby.CommanderOf(0) == 4,
    "commander standing down to NOAT falls to next-lowest (Dee, id 4)",
    $"NOAT stand-down wrong ({lobby.CommanderOf(0)})"
);
...
Check(
    lobby.CommanderOf(0) == -1,
    "emptied side has no commander (-1)",
    $"emptied side kept commander ({lobby.CommanderOf(0)})"
);
