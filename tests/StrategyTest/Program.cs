...
Check(
    content.Start.StartingCredits == 875,
    "Iron faction seeds 875 starting credits (1000 × 0.875)",
    $"starting credits wrong ({content.Start.StartingCredits})"
);
Check(
    content.Start.IncomePerPaycheck == 88,
    "Iron faction income is 88 per paycheck (100 × 0.875)",
    $"income wrong ({content.Start.IncomePerPaycheck})"
);
...
Check(
    world.TeamStates.Count == 2 && world.TeamStates.ContainsKey(0) && world.TeamStates.ContainsKey(1),
    "two team states keyed by team byte (0/1)",
    "team states not seeded for both teams"
);

var t0 = world.TeamStates[0];
var t1 = world.TeamStates[1];
Check(
    t0.Credits == 875 && t1.Credits == 875,
    "both teams start at 875 credits",
    $"seed credits wrong ({t0.Credits}/{t1.Credits})"
);
Check(t0.Score == 0 && t1.Score == 0, "both teams start at 0 score", "seed score wrong");
Check(
    t0.OwnedCapabilities.Contains(Capability.Base),
    "team 0 owns the Base capability (from faction seed)",
    "team 0 missing Base capability"
);
Check(
    t0.OwnedTechs.SetEquals(content.Start.BaseTechs),
    "team 0 owned techs equal the faction base techs",
    "team 0 owned techs mismatch faction seed"
);
...
Check(
    !t1.OwnedCapabilities.Contains(Capability.ShipyardAllowed) && !t1.OwnedTechs.Contains("test-tech"),
    "mutating team 0 owned sets does not affect team 1",
    "team owned sets are shared (not cloned per team)"
);
Check(
    !content.Start.BaseCapabilities.Contains(Capability.ShipyardAllowed) && !content.Start.BaseTechs.Contains("test-tech"),
    "mutating team 0 owned sets does not affect the faction snapshot",
    "team owned sets alias the faction snapshot"
);
...
for (int i = 0; i < (int)sim.PaycheckTicks + 5; i++)
    sim.Step();
Check(
    world.TeamStates[0].Credits == 875 && world.TeamStates[1].Credits == 875,
    "no credits accrue while in the lobby (even across a paycheck boundary)",
    $"credits changed in lobby ({world.TeamStates[0].Credits})"
);
...
sim.StartMatch();
Check(
    world.TeamStates[0].Credits == 875 && !world.TeamStates[0].OwnedCapabilities.Contains(Capability.ShipyardAllowed),
    "StartMatch re-seeds each team to starting credits + base unlocks",
    "StartMatch did not re-seed the economy"
);
...
Check(
    expectedPaychecks >= 2,
    "active stepping crossed at least two paycheck boundaries",
    $"only {expectedPaychecks} paychecks in window"
);
Check(
    world.TeamStates[0].Credits == expectedCredits,
...
Check(
    world.TeamStates[1].Credits == expectedCredits,
    "both teams accrue identically",
    $"team 1 credits {world.TeamStates[1].Credits} != team 0 {world.TeamStates[0].Credits}"
);
...
Check(
    unlocked.Contains(0) && !unlocked.Contains(1) && !unlocked.Contains(2),
...
Check(
    scoutCost > 0 && fighterCost > scoutCost && bomberCost > fighterCost,
    $"stock hull costs are sane (scout {scoutCost} < fighter {fighterCost} < bomber {bomberCost})",
    "hull costs not authored as expected"
);
...
Check(
    accept == Simulation.SpawnDecision.Allowed,
    "affordable unlocked Scout buy is Allowed",
    $"Scout buy not allowed ({accept})"
);
Check(
    world.TeamStates[0].Credits == 1000 - scoutCost,
    $"Scout buy deducts exactly its cost (1000 -> {1000 - scoutCost})",
    $"Scout deduct wrong: {world.TeamStates[0].Credits}"
);
...
Check(
    locked == Simulation.SpawnDecision.Locked,
    "locked hull buy is rejected (Locked)",
    $"locked buy not rejected ({locked})"
);
Check(
    world.TeamStates[0].Credits == beforeLocked,
    "a locked rejection deducts nothing",
    $"credits changed on locked reject: {world.TeamStates[0].Credits}"
);
...
Check(
    bomberLocked == Simulation.SpawnDecision.Locked,
    "an affordable but tech-locked Bomber buy is rejected (Locked)",
    $"tech-locked Bomber not rejected as Locked ({bomberLocked})"
);
Check(
    world.TeamStates[0].Credits == bomberCost + 500,
    "a tech-locked rejection deducts nothing",
    $"credits changed on tech-locked reject: {world.TeamStates[0].Credits}"
);
...
Check(
    poor == Simulation.SpawnDecision.TooPoor,
    "unaffordable Scout buy is rejected (TooPoor)",
    $"poor buy not rejected ({poor})"
);
Check(
    world.TeamStates[0].Credits == scoutCost - 1,
    "a too-poor rejection deducts nothing",
    $"credits changed on poor reject: {world.TeamStates[0].Credits}"
);
...
Check(
    devBomber.Price == 600 && devBomber.BuildTimeSeconds == 120,
    $"dev-bomber is 600cr / 120s",
    $"dev0 price/time wrong ({devBomber.Price}/{devBomber.BuildTimeSeconds})"
);
int bStartCredits = world.TeamStates[team0].Credits;
sim.EnqueueResearchOp(0, team0, Simulation.ResearchOpStart, base0Id, 0);
sim.Step();
Check(
    world.TeamStates[team0].Credits == bStartCredits - 600 + LastStepIncome(),
    "starting dev-bomber deducts exactly 600 credits",
    $"start deduct wrong (got {world.TeamStates[team0].Credits}, before {bStartCredits})"
);
Check(
    research0.Active.Count == 1 && research0.Active[0].DevIndex == 0,
    "the base has exactly one active research order after the start op",
    $"active state wrong (count {research0.Active.Count})"
);
...
Check(
    ...
    "bomber tech not granted on completion"
);
Check(
    world.TeamStates[team0].UnlockedClasses.Contains(2),
    "the bomber (class 2) is unlocked once dev-bomber completes",
    "bomber still locked after its gating research completed"
);
Check(
    cCompletedFlag,
...
Check(
    ...
    "the completed order is removed from the active list",
    $"active not cleared on completion ({research0.Active.Count})"
);
...
sim.Step();
Check(
    research0.Active.Count == 0 && world.TeamStates[team0].Credits == dBefore + LastStepIncome(),
...
sim.Step();
Check(
    research0.Active.Count == 1
        && research0.Active[0].DevIndex == 4
...
sim.StartMatch();
Check(
    content.Bases[0].ResearchSlots == 1,
    "garrison authors exactly 1 research slot",
    $"garrison research slots wrong ({content.Bases[0].ResearchSlots})"
);
...
sim.Step();
Check(
    research0.Active.Count == 1 && research0.Active[0].DevIndex == 3 && research0.OnDeck == 0,
    "with 1 slot: dev-minigun-2 runs active while dev-bomber waits on deck",
    $"slot/queue state wrong (active {research0.Active.Count}, ondeck {research0.OnDeck})"
);
Check(
    world.TeamStates[team0].Credits == eBefore - devMini2.Price - devBomber.Price + LastStepIncome(),
    "both the active AND the on-deck order deducted their price (queue = reservation)",
    $"queue reservation deduct wrong ({world.TeamStates[team0].Credits})"
);
...
sim.Step();
Check(
    research0.Active.Count == 1 && research0.OnDeck == 0 && world.TeamStates[team0].Credits == e3Before + LastStepIncome(),
    "a third start is rejected while the base is fully occupied (no charge, state unchanged)",
    $"occupancy rejection wrong (active {research0.Active.Count}, ondeck {research0.OnDeck}, credits {world.TeamStates[team0].Credits})"
);
...
while (sim.Tick < eComplete)
    sim.Step(); // the eComplete-tick step completes dev-minigun-2 AND promotes dev-bomber
Check(
    research0.Active.Count == 1
        && research0.Active[0].DevIndex == 0
        && research0.Active[0].StartTick == eComplete
        && research0.OnDeck == null,
    "on completion the on-deck order auto-promotes to active with a fresh StartTick",
    $"promotion wrong (active {research0.Active.Count}, dev {(research0.Active.Count > 0 ? research0.Active[0].DevIndex : -1)}, start {(research0.Active.Count > 0 ? research0.Active[0].StartTick : 0)} vs {eComplete}, ondeck {research0.OnDeck})"
);
...
sim.Step();
Check(
    research0.Active.Count == 0 && world.TeamStates[team0].Credits == fAfterStart + devBomber.Price + LastStepIncome(),
    "cancel-active refunds the full price and clears the active order",
    $"cancel-active wrong (active {research0.Active.Count}, credits {world.TeamStates[team0].Credits} vs {fAfterStart + devBomber.Price})"
);
...
Check(
    research0.OnDeck == null
        && research0.Active.Count == 1
        && research0.Active[0].DevIndex == 3
        && world.TeamStates[team0].Credits == fAfterQueue + devBomber.Price + LastStepIncome(),
    "cancel-on-deck refunds the reservation and clears the on-deck slot (active untouched)",
    $"cancel-on-deck wrong (ondeck {research0.OnDeck}, active {research0.Active.Count}, credits {world.TeamStates[team0].Credits} vs {fAfterQueue + devBomber.Price})"
);
...
sim.Step();
Check(
    research0.Active.Count == fActiveBefore && world.TeamStates[team0].Credits == fNoopBefore + LastStepIncome(),
    "cancelling a development that isn't present is a no-op (no refund, no state change)",
    $"cancel no-op wrong (active {research0.Active.Count} vs {fActiveBefore}, credits {world.TeamStates[team0].Credits} vs {fNoopBefore})"
);
...
sim.Step();
Check(
    research0.Active.Count == 1 && research0.OnDeck == null && world.TeamStates[team0].Credits == gBefore + LastStepIncome(),
    "starting a development already in progress is rejected (no charge, not queued on deck)",
    $"duplicate rejection wrong (active {research0.Active.Count}, ondeck {research0.OnDeck}, credits {world.TeamStates[team0].Credits} vs {gBefore})"
);
...
sim.Step();
Check(
    research0.Active.Count == 0 && world.TeamStates[team0].Credits == hBefore + LastStepIncome(),
    "starting a development the team can't afford is rejected with no charge",
    $"insufficient-credit rejection wrong (active {research0.Active.Count}, credits {world.TeamStates[team0].Credits} vs {hBefore})"
);

// ---- i. IsPlayerSpawnableClass: the def-driven MsgSpawn class gate (availability != projection). ----
Check(
    sim.IsPlayerSpawnableClass(0) && sim.IsPlayerSpawnableClass(1) && sim.IsPlayerSpawnableClass(2),
...
Check(
    !sim.IsPlayerSpawnableClass(255) && !sim.IsPlayerSpawnableClass(4),
...
Check(
    research0.Active.Count == 1,
    "research is active going into the match restart",
    "expected active research before restart"
);
sim.ReturnToLobby();
sim.StartMatch();
Check(
    world.ResearchByBase.All(r => r.Active.Count == 0 && r.OnDeck == null),
    "StartMatch clears all in-flight research across every base (ResearchByBase empty)",
    "research survived a match restart"
);
Check(
    world.TeamStates[team0].Credits == content.Start.StartingCredits,
    "StartMatch re-seeds team credits to the starting amount",
    $"credits not re-seeded on restart ({world.TeamStates[team0].Credits})"
);
...
bool Offered(string devId) =>
    Allegiance
        .Factions.Resolution.BuildableResolver.GetBuildables(content.Catalog, tsU.OwnedTechs, tsU.OwnedCapabilities)
...
Check(
    worldU.Bases[uBaseIdx].BaseTypeId == 0 && worldU.BaseMaxHealthOf(4) == 2500f,
    "garrison starts as base type 0 and the type-4 tier resolves 2500 max armor",
    $"setup wrong (type {worldU.Bases[uBaseIdx].BaseTypeId}, type4 max {worldU.BaseMaxHealthOf(4)})"
);
Check(
    !Offered("dev-minigun-3"),
    "dev-minigun-3 is locked before the garrison upgrade (needs garrison-str)",
    "dev-minigun-3 wrongly offered pre-upgrade"
);
RunToCompletion(7, uBaseId); // dev-upgrade-garrison
Check(
    worldU.Bases[uBaseIdx].BaseTypeId == 4,
    "researching dev-upgrade-garrison swaps the garrison to type 4 (Garrison Str)",
    $"garrison not upgraded (type {worldU.Bases[uBaseIdx].BaseTypeId})"
);
Check(
    MathF.Abs(worldU.BaseHealth[uBaseIdx] - 1250f) < 1f,
    "garrison health rescaled by fraction-of-max into the new tier (frac preserved: 1000/2300 × 2875 = 1250; the ×1.15 team factor cancels)",
    $"health rescale wrong (health {worldU.BaseHealth[uBaseIdx]})"
);
Check(
    tsU.OwnedTechs.Contains("garrison-str"),
    "the upgrade grants the garrison-str tech team-wide",
    "garrison-str tech not granted"
);
Check(
    content.Bases.First(b => b.BaseTypeId == 4).WinCondition,
...
Check(
    Offered("dev-minigun-3"),
    "dev-minigun-3 becomes researchable once garrison-str is owned",
    "dev-minigun-3 not offered after the garrison upgrade"
);
...
Check(
    worldU.Bases[supBaseIdx].BaseTypeId == 5,
    "researching dev-upgrade-supremacy at the Supremacy swaps it to type 5 (Adv)",
    $"supremacy not upgraded (type {worldU.Bases[supBaseIdx].BaseTypeId})"
);
Check(
    tsU.OwnedTechs.Contains("supremacy-adv"),
    "the supremacy upgrade grants the supremacy-adv tech",
    "supremacy-adv tech not granted"
);
Check(
    Offered("dev-gat-3"),
    "dev-gat-3 becomes researchable once supremacy-adv is owned",
    "dev-gat-3 not offered after the supremacy upgrade"
);
...
simU.Step();
Check(
    worldU.ResearchByBase[supBaseIdx].Active.Exists(a => a.DevIndex == 5)
...
Check(
    worldU.Bases[rIdx].BaseTypeId == 0 && worldU.BaseHealth[rIdx] == worldU.BaseMaxHealthOf(0, worldU.Bases[rIdx].Team),
...
// ============================================================================
//  LAUNCH-STATION-CLASSES (2026-07-21) — TryResolveLaunchSite spawn gate + the dock class gate.
//  The Devastator authors launch-station-classes: [shipyard] (LaunchClassMask 1 << Shipyard(2)).
//  Test bases here are MODEL-LESS (no GLB assets next to the test binary), so every base is
//  launch-CAPABLE via the legacy sphere fallback — these tests exercise the CLASS gate.
// ============================================================================
{
    var worldL = new World(4242, content.World, content.Bases[0].MaxHealth, content.Start, content.Ships);
    var simL = new Simulation(worldL, content);
    simL.MinersEnabled = false;
    simL.StartMatch();
    var tsL = worldL.TeamStates[0];
    int LInc() => (simL.Tick % simL.PaycheckTicks == 0) ? content.Start.IncomePerPaycheck : 0;

    var devDef = content.Ships.First(s => s.Name == "Devastator");
    byte devCls = devDef.ClassId;
    Check(
        devDef.LaunchClassMask == 1 << 2,
        "Devastator projects launch-station-classes: [shipyard] to LaunchClassMask 1 << 2",
        $"Devastator LaunchClassMask wrong (got {devDef.LaunchClassMask})"
    );
    Check(
        content.Ships.First(s => s.ClassId == 0).LaunchClassMask == 0,
        "Scout stays unrestricted (LaunchClassMask 0)",
        "Scout unexpectedly gained a launch restriction"
    );

    int garIdx = worldL.Bases.FindIndex(b => b.Team == 0);
    ulong garId = worldL.Bases[garIdx].Id;
    var garPos = worldL.Bases[garIdx].Pos;
    uint garSector = worldL.Bases[garIdx].SectorId;
    ulong yardId = worldL.CreateBase(
        0,
        3 /* shipyard */
        ,
        garSector,
        garPos + new Vec3(600f, 0f, 0f)
    );
    var yardPos = worldL.Bases.First(b => b.Id == yardId).Pos;
    tsL.UnlockedClasses.Add(devCls); // bypass the tech chain — this section tests the BASE gate

    // 1. Devastator from the GARRISON: rejected BEFORE the charge — no ship, no credit movement.
    tsL.Credits = 10000;
    simL.EnqueueJoin(41, 0, devCls, System.Array.Empty<(uint, byte)>(), garId);
    simL.Step();
    Check(
        !simL.Ships.Any(s => s.OwnerClientId == 41) && tsL.Credits == 10000 + LInc(),
        "Devastator spawn at the garrison is rejected pre-charge (no ship, no deduction)",
        $"garrison launch not rejected cleanly (ship {simL.Ships.Any(s => s.OwnerClientId == 41)}, credits {tsL.Credits})"
    );

    // 2. Devastator from the SHIPYARD: spawns and deducts exactly its cost.
    tsL.Credits = 10000;
    int lBefore = tsL.Credits;
    simL.EnqueueJoin(42, 0, devCls, System.Array.Empty<(uint, byte)>(), yardId);
    simL.Step();
    var dev42 = simL.Ships.FirstOrDefault(s => s.OwnerClientId == 42);
    Check(
        dev42 is not null && tsL.Credits == lBefore - devDef.Cost + LInc(),
        $"Devastator spawn at the shipyard succeeds and deducts {devDef.Cost}",
        $"shipyard launch failed (ship {dev42 is not null}, credits {tsL.Credits} vs {lBefore - devDef.Cost})"
    );
    bool nearYard42 = dev42 is not null && (dev42.State.Pos - yardPos).Length() < (dev42.State.Pos - garPos).Length();
    Check(
        nearYard42,
        "the shipyard-launched Devastator is placed at the shipyard, not the garrison",
        "Devastator placed at the wrong base"
    );

    // 3. launchBaseId 0 (default pick): the restricted hull lands on the SHIPYARD, never "first base".
    if (dev42 is not null)
        dev42.State.Pos = yardPos + new Vec3(0f, 2000f, 0f); // park clear so the next spawn can't collide
    tsL.Credits = 10000;
    simL.EnqueueJoin(43, 0, devCls, System.Array.Empty<(uint, byte)>(), 0);
    simL.Step();
    var dev43 = simL.Ships.FirstOrDefault(s => s.OwnerClientId == 43);
    Check(
        dev43 is not null && (dev43.State.Pos - yardPos).Length() < (dev43.State.Pos - garPos).Length(),
        "a default-base (0) Devastator spawn resolves to the shipyard, not the first team base",
        $"default pick wrong (ship {dev43 is not null})"
    );

    // 4. Dock class gate on the REAL base models (SimAssets probes up to the repo GLBs from the
    // test binary, so garrison=ss27 and shipyard=acs05 doors are live here). A restricted hull:
    //   a) does NOT dock even dead-centre in a GARRISON door (wrong station class — solid),
    //   b) does NOT dock in the shipyard's SMALL top door (largest-door-only),
    //   c) DOES dock in the shipyard's LARGEST door (despawn + PaidCost refund).
    if (dev43 is not null)
        dev43.State.Pos = yardPos + new Vec3(0f, -2000f, 0f); // park clear of the dock probes below
    var garDoors = worldL.BaseDockFacesOf(worldL.Bases[garIdx].BaseTypeId);
    var yardDoors = worldL.BaseDockFacesOf(3);
    int yardLargest = worldL.BaseLargestDockFaceOf(3);
    Check(
        garDoors.Length > 0 && yardDoors.Length == 2 && yardLargest >= 0,
        "real base models loaded (garrison doors + 2 shipyard doors parsed)",
        $"expected live GLB doors (gar {garDoors.Length}, yard {yardDoors.Length}, largest {yardLargest})"
    );
    if (dev42 is not null && garDoors.Length > 0 && yardDoors.Length == 2)
    {
        ...
        Check(
            ...
            "a Devastator dead-centre in a garrison door does NOT dock (wrong class — solid base)",
        ...
        Check(
            simL.Ships.Any(s => s.OwnerClientId == 42),
            "a Devastator in the shipyard's SMALL door does NOT dock (largest-door-only)",
            "Devastator wrongly docked through the shipyard side door"
        );
        ...
        Check(
            ...
            $"shipyard dock failed (ship {simL.Ships.Any(s => s.OwnerClientId == 42)}, credits {tsL.Credits} vs {dockBefore + devDef.Cost})"
        );
    }

    // 5. Legacy regression: unrestricted hulls keep today's behaviour — a Scout launches from the
    // garrison, and a Scout with a STALE base id silently falls back to a team base (no rejection).
    tsL.Credits = 10000;
    simL.EnqueueJoin(44, 0, 0, System.Array.Empty<(uint, byte)>(), garId);
    simL.Step();
    Check(
        simL.Ships.Any(s => s.OwnerClientId == 44),
        "a Scout still launches from the garrison",
        "Scout garrison launch broke"
    );
    simL.EnqueueJoin(45, 0, 0, System.Array.Empty<(uint, byte)>(), 999_999_999UL);
    simL.Step();
    Check(
        simL.Ships.Any(s => s.OwnerClientId == 45),
        "a Scout with a stale base id silently falls back (legacy)",
        "stale-id fallback for an unrestricted hull broke"
    );
}
