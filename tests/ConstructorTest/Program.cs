...
Check(
    garrisonDef.WinCondition && !outpostDef.WinCondition,
...
Check(
    outpostDef.BuildRockClass == (byte)RockClass.Regolith && outpostDef.ModelName == "ss90",
...
Check(
    supremacyCat.BuildRockClass == (byte)RockClass.Carbonaceous,
...
Check(
    !supremacyCat.RequiredCaps.Contains((byte)CapabilityId.ExpansionAllowed)
...
Simulation.ShipSim? ConstructorShip(Simulation sim) =>
    sim.ConstructorSlotsView().Count > 0 ? sim.ConstructorSlotsView()[0].Ship : null;
string ConstructorState(Simulation sim) =>
    sim.ConstructorSlotsView().Count > 0 ? sim.ConstructorSlotsView()[0].State : "(none)";
...
// ---- Scenario 1: buy → charge + PRODUCE, then launch (v38) --------------------------------------
{
    ...
    Check(
        sim.ConstructorCount(0) == 1 && ConstructorShip(sim) is null && ConstructorState(sim) == "Producing",
    ...
    Check(
        world.TeamStates[0].Credits == credits0 - outpostPriceConst,
    ...
    Check(
        sim.ConstructorCount(0) == 1 && ship is { Kind: ShipKind.Constructor, Alive: true },
...
// ---- Scenario 1b: cancel while producing → refund + removed ------------------------------------
{
    ...
    sim.Step();
    Check(
        sim.ConstructorCount(0) == 0 && world.TeamStates[0].Credits == credits0,
...
// ---- Scenario 1c: build pipeline — parallel=1 serializes, queue=2 caps, launch promotes ----------
{
    ...
    Check(
        sim.ConstructorSlotsView().Count == 2 && Producing() == 1 && Queued() == 1,
    ...
    sim.Step();
    Check(
        sim.ConstructorCount(0) == 2 && world.TeamStates[0].Credits == creditsBefore3,
...
// silently reverted to defaults — so an authored build.parallel-limit was lost on every mapped game.
{
    ...
    Check(
        cloned.Build.ParallelLimit == 1 && cloned.Build.QueueLimit == 2 && cloned.Constructor.ProductionSeconds == 99f,
...
// garrison must NOT both build at once (they share one per-garrison pipeline).
{
    ...
    Check(
        sim.ConstructorStatesView().Count == 2 && prod == 1 && queued == 1,
...
// the drone actually launches.
{
    ...
    Check(
        producing.State == 0 && producing.ShipId == 0,
    ...
    Check(
        ship is Simulation.ShipSim s && launched.ShipId == s.ShipId && launched.StationType == OutpostType,
...
// ---- Scenario 2: order to a Regolith rock → base appears, drone consumed ------------------------
{
    ...
    if (ship is Simulation.ShipSim sh2 && regolith is ulong rockId)
    {
        ...
        Check(
            built,
            "a constructor ordered to a Regolith rock raises a base there",
        ...
        if (built)
        {
            ...
            Check(
                nb.BaseTypeId == OutpostType && nb.Team == 0 && atRock && sim.RockHasBase(rockId),
            ...
            for (int i = 0; i < 30 && world.RockById(rockId) is not null; i++)
                sim.Step();
            Check(
                world.RockById(rockId) is null && !world.Asteroids.Any(r => r.Id == rockId),
...
// ---- Scenario 3: rock-class gate — a He3 rock is refused ---------------------------------------
{
    ...
    if (ship is Simulation.ShipSim sh3 && he3 is ulong rockId)
    {
        ...
        for (int i = 0; i < 200; i++)
            sim.Step();
        Check(
            world.Bases.Count == garrisons && sim.ConstructorCount(0) == 1,
...
// outside the aleph trigger; the park band dipped back inside and the drone warped straight home.
{
    ...
    if (ship is Simulation.ShipSim sh4b && gateOut is World.Gate gate)
    {
        ...
        Check(
            warpTick >= 0,
            "a sector-ordered constructor warps into the ordered sector",
        ...
        Check(
            stayed && ConstructorState(sim) == "MoveTo",
...
// constructor abandoning its post and returning home.
{
    ...
    if (ship is Simulation.ShipSim sh4c && gateOut is World.Gate gate)
    {
        ...
        Check(
            stayed && ConstructorState(sim) == "Idle",
...
// Supremacy runs 2 concurrent research slots while the garrison runs 1. -------------------------------
{
    ...
    Check(
        carb is not null,
        "a seed with a Carbonaceous rock exists (40-seed scan)",
    ...
    var regoliths = world.Asteroids.Where(r => world.RockClassOf(r.Id) == RockClass.Regolith).Select(r => r.Id).ToList();
    Check(
        regoliths.Count >= 2,
        "the map has >=2 Regolith rocks to expand onto",
    ...
    Check(
        outpostIdx >= 0 && world.Bases[outpostIdx].BaseTypeId == OutpostType,
        "an outpost builds on a Regolith rock",
        $"outpost build failed (idx {outpostIdx})"
    );
    Check(
        outpostIdx >= 0 && Math.Abs(world.BaseHealth[outpostIdx] - 667f) < 0.5f,
    ...
    Check(
        supIdx >= 0 && world.Bases[supIdx].BaseTypeId == SupremacyType,
        "a Supremacy Center builds on a Carbonaceous rock",
        $"supremacy build failed (idx {supIdx})"
    );
    Check(
        supIdx >= 0 && Math.Abs(world.BaseHealth[supIdx] - 1333f) < 0.5f,
    ...
    Check(
        world.TeamStates[0].OwnedTechs.Contains("supremacy-1"),
    ...
    Check(
        world.TeamStates[0].UnlockedClasses.Contains((byte)1),
    ...
    Check(
        sim.TryReserveSpawn(0, 1) == Simulation.SpawnDecision.Allowed,
    ...
    sim.Step();
    Check(
        world.ResearchByBase[supIdx].Active.Count == 2 && world.ResearchByBase[supIdx].OnDeck is null,
    ...
    sim.Step();
    Check(
        world.ResearchByBase[garIdx].Active.Count == 1,
    ...
    Check(
        yardIdx >= 0 && world.Bases[yardIdx].BaseTypeId == ShipyardType,
        "a Shipyard builds on a Regolith rock",
        $"shipyard build failed (idx {yardIdx})"
    );
    Check(
        world.TeamStates[0].OwnedTechs.Contains("shipyard-1"),
...
// first 2 Hz apply. Synchronous vision (FogTest idiom) makes each boundary compute inline. ----------
{
    ...
    if (carb is ulong carbId)
    {
        ...
        Check(
            (world.TeamStates[0].DiscoveredRockClasses & (1 << (byte)RockClass.Carbonaceous)) != 0,
        ...
        sim.Step();
        Check(
            sim.ConstructorCount(0) == 2 && world.TeamStates[0].Credits < creditsPre,
...
// ---- Scenario 7: fog OFF stamps the mask full — no rock gate without fog -------------------------
{
    ...
    Check(
        world.TeamStates[0].DiscoveredRockClasses == 0xFF,
...
// with an explicit special-count bypasses the home-special-chance roll, so this seeds specials. -----
{
    ...
    var carbOnly = new SpecialWeights
    {
        Carbonaceous = 1f,
        Silicon = 0f,
        Uranium = 0f,
    ...
    var silOnly = new SpecialWeights
    {
        Carbonaceous = 0f,
        Silicon = 1f,
        Uranium = 0f,
    ...
    for (ulong seed = 1; seed <= 20; seed++)
    {
        ...
        foreach (var r in bw.Asteroids)
        {
            ...
            if (cls == RockClass.Carbonaceous)
                carb2++;
            else if (cls is RockClass.Silicon or RockClass.Uranium)
                other2++;
        }
        if (carb2 < 1 || other2 != 0)
        {
