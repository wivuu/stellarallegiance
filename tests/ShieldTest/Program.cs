...
(Simulation sim, Simulation.ShipSim attacker, Simulation.ShipSim target) SetupDuel(
    ulong seed,
    byte attackerCls,
    byte targetCls,
    float dist
...
// ---- 1. Spawn capacity from the class def ------------------------------------------------------
{
    ...
    Check(
        fighterCap > 0f && Near(attacker.Shield, fighterCap),
    ...
    Check(
        scoutCap > 0f && Near(target.Shield, scoutCap),
...
// regen inside the delay window, then regen at the authored rate (points/sec) and clamp to capacity.
{
    ...
    Check(
        Near(s.Shield, 20f),
        "shield does not recharge during the quiet delay",
        $"shield recharged early ({s.Shield}, expected 20)"
    );
    ...
    for (uint i = 0; i < window; i++)
        sim.Step();
    Check(
        Near(s.Shield - before, window * perTick),
...
// exactly one bolt lands: it must remove only 0.5*Damage from the target's shield, hull untouched.
{
    ...
    Check(
        Near(cannon.ShieldMult, 0.5f),
        $"AutoCan 1 carries the authored shield-mult 0.5 ({cannon.ShieldMult})",
        $"AutoCan shield-mult wrong ({cannon.ShieldMult})"
    );
    ...
    Check(
        hitSeen && Near(target.Shield, cap - cannon.Damage * cannon.ShieldMult),
...
// the bolt-fire seam (shooter team 0), stacking correctly with the per-weapon shield multiplier.
{
    ...
    Check(
        hitSeen && Near(target.Shield, cap - ironShieldHit),
...
// exercises the ResolveDueShots heal branch with ShieldsEnabled on (the whole point of "bypasses").
{
    ...
    Check(
        nanite.IsHealing && nanite.Damage > 0f,
        $"nanite is a healing gun (heal power {nanite.Damage})",
        $"nanite not healing (heal {nanite.IsHealing}, power {nanite.Damage})"
    );
    ...
    Check(
        Near(target.Shield, shieldFull) && target.ShieldDamageTick == shieldTickBefore,
...
// ---- 7. Nanite heal clamps at MaxHull (never overshoots) ---------------------------------------
{
    ...
    Check(
        healed && Near(target.Health, maxHull),
...
// ---- 8. Enemy hit by a nanite bolt takes ZERO effect (no damage, no shield change) --------------
{
    ...
    Check(
        Near(enemy.Health, 60f) && Near(enemy.Shield, 0f),
