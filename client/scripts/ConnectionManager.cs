...
public partial class ConnectionManager : Node
{
    ...
    // Static (no instance state) so AuthSession can resolve the same base URL before — or without —
    // a live ConnectionManager, e.g. to decide whether a persisted user://auth.json session still
    // matches the lobby this launch is pointed at.
    public static string ResolveLobbyBase()
    {
        string lobby = "";
        var cmd = OS.GetCmdlineArgs();
        for (int i = 0; i < cmd.Length; i++)
        {
            if (cmd[i] == "--lobby" && i + 1 < cmd.Length)
                lobby = cmd[i + 1];
        ...
        if (string.IsNullOrEmpty(lobby))
            lobby = DefaultLobby;
        return lobby.StartsWith("http") ? lobby.TrimEnd('/') : $"http://{lobby}";
    }
